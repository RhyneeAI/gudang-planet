...
it('returns 422 when name exceeds 255 characters', function () {
    $this->actingAs($this->user)
        ->postJson('/api/v1/customers', ['name' => str_repeat('a', 256)])
        ->assertStatus(422);
});
...
it('returns 401 when not authenticated on store', function () {
    $this->postJson('/api/v1/customers', ['name' => 'Budi Santoso'])
        ->assertStatus(401);
});
...
it('returns 404 when customer not found on show', function () {
    $this->actingAs($this->user)
        ->getJson('/api/v1/customers/uuid-tidak-ada')
        ->assertStatus(404);
});
...
it('returns 404 when updating non-existent customer', function () {
    $this->actingAs($this->user)
        ->patchJson('/api/v1/customers/invalid-uuid', ['name' => 'New Name'])
        ->assertStatus(404);
});
...
it('returns 404 when deleting non-existent customer', function () {
    $this->actingAs($this->user)
        ->deleteJson('/api/v1/customers/invalid-uuid')
        ->assertStatus(404);
});
