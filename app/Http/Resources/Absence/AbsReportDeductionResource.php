...
class AbsReportDeductionResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        $subCompany = $this->attendance?->subCompany
            ?? $this->user?->absEmployeeProfile?->subCompany;

        return [
            'ulid' => (string) $this->ulid,
            'date' => $this->attendance?->date?->toDateString() ?? $this->created_at?->toDateString(),
            'reason' => $this->reason,
